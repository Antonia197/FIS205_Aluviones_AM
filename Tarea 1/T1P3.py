# a) El modelo matemático para este problema será tal que
#  sus ejes de referencia son Z apuntará hacia arriba, x hacia el este e y hacia el norte
# Por esto P=-mgz tongo, el roce aerodinámico es F_a= -1/2 rho C_d A |v|v
# Y como nos mencionan que la Tierra está rotando, entonces también debemos considerar la fuerza de Coriolis
# F_c = -2m (omega x v) donde omega es la velocidad angular de la Tierra y v es la velocidad del proyectil
# Por lo tanto, la ecuación diferencial que describe el movimiento del proyectil será tal que
# dv/dt= g- 1/2m rho C_d A |v|v - 2 (omega x v)
# CI Misil 1: se lanza t=0; r_1,0=(0,0,0) v_1,0= (vx, vy,vz), teta_1=45 grados, vx=500*cos(teta_1)*sin(azimut_1), vy=500*cos(teta_1)*cos(azimut_1), azimut_1= 30 grados, vz=500*sin(teta_1)
# CI Misil 2: se lanza tau=10s; r_2,0=(5000,2000,0) sera útil d_tol=|r_1(t)-r_2(t)1<10 m
